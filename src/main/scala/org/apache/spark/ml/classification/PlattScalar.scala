...
/**
  ...
  */
final class PlattScalarModel(override val uid: String, val models: Array[BinaryLogisticRegressionWithDoubleResponseModel])
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    if($(isMultiIntent)) {
      ...
      // This column will used to calculate probability column for OVR model
      ...
      val aggregatedDataset = models.zipWithIndex.foldLeft[DataFrame](newDataset) {
        case (df, (model, index)) =>
          val probabilityCol = model.getProbabilityCol
          ...
          val transformedDataset = model.transform(plattFeaturesDataset).select(columns: _*)
          val updatedDataset = transformedDataset.withColumn(tmpColName, updateUDF(col(accColName), col(probabilityCol)))
      ...
      val mapToVectorUDF = udf { predictions: Map[Int, Double] =>
        Vectors.dense(predictions.toSeq.sortBy(_._1).map(_._2).toArray[Double])
      }

      aggregatedDataset
        .withColumn(accColName + "_tempVec", mapToVectorUDF(col(accColName)))
        .withColumnRenamed(accColName + "_tempVec", PLATT_PROBABILITY_COLUMN)
        .drop(accColName)
    }
    else {
...
object PlattScalarModel extends MLReadable[PlattScalarModel] {
  ...
  private[PlattScalarModel] class PlattScalarModelWriter(instance: PlattScalarModel) extends MLWriter {
    ...
    override protected def saveImpl(path: String): Unit = {
      val extraJson = if(instance.getIsMultiIntent) Some(JsonAST.JObject(List("numClasses" -> JInt(instance.models.length)))) else None
  ...
  private class PlattScalarModelReader extends MLReader[PlattScalarModel] {
    ...
    override def load(path: String): PlattScalarModel = {
      ...
      val models = if (isMultiIntent) {
        ...
        Range(0, numClasses).toArray.map { idx =>
          val modelPath = new Path(path, s"binlrModel_$idx").toString
...
/**
  * PlattScaler calibrates the score for an SVM classifier to fall into a [0, 1] range and can be used as a probability score.
  ...
  * <br />
  * More information: <br />
  *   [1] Platt, J. C. Probabilistic Outputs for Support Vector Machines and Comparisons to Regularized Likelihood Methods.
...
final class PlattScalar(override val uid: String)
  ...
  override def fit(dataset: Dataset[_]): PlattScalarModel = instrumented { instr =>
    ...
    val models = if ($(isMultiIntent)) {
      ...
      var bayesTargetRevisionFailureCount = 0
      val maxFailureCount = 5
      ...
      val modelFutures = Range(0, numClasses).map { index =>
        ...
        Future
        {
          ...
          val newLabelMeta: Metadata = BinaryAttribute.defaultAttr.withName("label").toMetadata()
          val labelColName = "response$" + index
          // Set up the binary response column
          val classLevelDataset = rawPredictionDf.withColumn(
          ...
          // Now we will update the targets of {0,1} for class labels to a Bayes-inspired revised target, based
          // on the original paper from by Platt [1]. This updated dataset is however prone to failure to fit to
          // an LR model, mostly becuase of failure in linesearch. We will keep a count of failure separately, and
          // if the update has already failed for 5 models, we will not bother: will just fit to a simple LR model
          // instead.
          val (fitNeeded, fittedModel) = if(bayesTargetRevisionFailureCount < maxFailureCount)
          {
            ...
            val updatedClassLevelDS = classLevelDataset.withColumn(labelColName, when(col(labelColName) === 0.0, lowerVal).otherwise(upperVal))
            ...
            import com.tfs.test.util.Implicits._
            val model = binLR.fit(trainingDataset)
            if(model.coefficients(0) == 0d)
            {
              bayesTargetRevisionFailureCount += 1
              logger.info("Failure to fit an LR model with revised target.")
            }

            (model.coefficients(0) == 0d, model)
          } else (true, null)

          // Check if we have successfully fit a BinaryLogisticRegressionWithDoubleResponse
          // Often this fails with LineSearch issues. In those cases, we will revert back to
          // fitting a vanilla binary LR model.
          if(fitNeeded)
          {
            logger.info(s"${if(bayesTargetRevisionFailureCount >= maxFailureCount) "Exceeded failure count while fitting an LR model with revised target. " else ""}Falling back on fitting to original dataset.")
            // Refit with LR, by using the unmodified classLevelDataset
            val newTrainingDataset = buildPlattFeatures(classLevelDataset, index)
            val newLR = new BinaryLogisticRegressionWithDoubleResponse()
              .setRegParam(0.1)
              .setFeaturesCol(PLATT_FEATURES_COLUMN)
              .setRawPredictionCol(PLATT_SCORE_COLUMN)
              .setProbabilityCol(PLATT_PROBABILITY_COLUMN)
