...
/** Transformer to replace regexes with a particular string
  *
...
class RegexReplacementTransformer(override val uid: String)
  ...
  private def regexReplace(line:String):String = {
    //var resultantLine = line
    //todo use fold
    /*sortedRegexMapSeq.foreach(regexTuple => {
      val regexObj = regexTuple._2.r
      resultantLine = regexObj.replaceAllIn(resultantLine, regexTuple._1.substring(1))
    })*/
    getRegexReplacements.foldLeft(line)((accumulator, regexTuple) => {
      val regexObj = regexTuple._2.r
